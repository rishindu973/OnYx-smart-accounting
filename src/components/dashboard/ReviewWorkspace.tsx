...
"use client";
import { useState, useEffect } from "react";
import { CheckCircle, AlertTriangle, FileText, ZoomIn, ZoomOut, Sparkles, UserPlus, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Badge } from "@/components/ui/badge";
...
const ReviewWorkspace = ({ data: initialData }: { data: UniversalDocument }) => {
  ...
  useEffect(() => {
    // ✅ Load evidence only for AI scans [cite: 851, 1139]
  ...
  // ✅ Confidence Heatmap: highlight < 90% in red 
  const getFieldStyle = (fieldName: keyof typeof formData.intelligence.confidence_score) => {
    if (formData.metadata.isManual) return "";
    const score = formData.intelligence.confidence_score[fieldName];
    if (score >= 0.95) return "border-success/40 bg-success/5"; // Verified [cite: 853]
    if (score >= 0.90) return "border-warning/40 bg-warning/5"; // Normal Review
    return "bg-destructive/10 border-destructive/30 focus:border-destructive"; // Low Confidence [cite: 853, 1023]
  };
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex overflow-hidden">
      {/* LEFT SIDE: Evidence (Hidden if Manual)  */}
      {!formData.metadata.isManual && (
        <div className="w-1/2 border-r border-border flex flex-col bg-card overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between">
          ...
          <div className="flex-1 overflow-auto p-4 flex items-start justify-center bg-muted/20">
            {imagePreview ? (
              <img src={imagePreview} className="max-w-full h-auto shadow-2xl rounded-lg origin-top transition-transform duration-200" style={{ transform: `scale(${zoom / 100})` }} />
            ) : <AlertTriangle className="w-12 h-12 text-muted-foreground mt-20" />}
          </div>
        </div>
      )}
      ...
      <div className={`${formData.metadata.isManual ? 'w-full max-w-2xl mx-auto' : 'w-1/2'} flex flex-col bg-background`}>
        <div className="p-4 border-b border-border flex items-center justify-between font-medium">
          <span className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary" /> {formData.metadata.source === "AI_SCAN" ? "AI Extraction Result" : "User Entry Result"}</span>
          <Badge variant="secondary">{formData.metadata.source}</Badge>
        </div>

        <div className="flex-1 p-6 overflow-auto space-y-6">
          {/* ✅ Zero-Click Provisioning Alert [cite: 1025, 1149] */}
          {showVendorAlert && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex items-start gap-4">
              <UserPlus className="w-10 h-10 p-2 rounded-full bg-primary/10 text-primary" />
              <div className="flex-1">
                <h4 className="font-semibold text-sm">New Vendor Detected</h4>
                <p className="text-xs text-muted-foreground mb-3">"{formData.extracted_data.payee_name}" is not in your vendor list.</p>
                <Button size="sm" onClick={() => setShowVendorAlert(false)}>Create Draft Account</Button>
              </div>
            </motion.div>
          )}
          ...
          <div className="space-y-5">
            <div className="space-y-2">
              <Label>Payee</Label>
              <Input value={formData.extracted_data.payee_name} className={getFieldStyle('payee_name')} onChange={(e) => setFormData({...formData, extracted_data: {...formData.extracted_data, payee_name: e.target.value}})} />
            </div>
            <div className="space-y-2">
              <Label>Amount (Rupees)</Label>
              <Input type="number" value={formData.extracted_data.total_amount} className={getFieldStyle('amount_numeric')} onChange={(e) => setFormData({...formData, extracted_data: {...formData.extracted_data, total_amount: parseFloat(e.target.value)}})} />
            </div>
            {/* ✅ Agentic Check: Word Amount Cross-Validation  */}
            {formData.metadata.type === "CHEQUE" && (
              <div className="space-y-2">
                <Label>Amount in Words</Label>
                <Input value={formData.extracted_data.amount_in_words} className={getFieldStyle('amount_in_words')} onChange={(e) => setFormData({...formData, extracted_data: {...formData.extracted_data, amount_in_words: e.target.value}})} />
              </div>
            )}
          </div>
          ...
          <div className={`p-4 rounded-xl border ${formData.intelligence.amount_validation_passed ? 'bg-success/10 border-success/30' : 'bg-destructive/10 border-destructive/30'}`}>
            <p className="font-semibold text-sm">{formData.intelligence.amount_validation_passed ? 'AI Logic Verified' : 'Manual Review Suggested'}</p> [cite: 896, 899]
          </div>
        </div>
